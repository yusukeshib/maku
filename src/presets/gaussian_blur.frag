...
void main() {
  ...
  for (dx = 1.0 / u_resolution.x, x=-u_radius, p.x = 0.5+(pos.x*0.5)+(x*dx);x<=u_radius;x++,p.x+=dx) {
    xx = x * x;
    for (dy=1.0 / u_resolution.y,y=-u_radius,p.y=0.5+(pos.y*0.5)+(y*dy);y<=u_radius;y++,p.y+=dy){
      yy=y*y;
      if (xx+yy<=rr) {
