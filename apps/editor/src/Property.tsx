...
export const Property = memo(function Property({ propId }: { propId: NodeId }) {
  ...
  return (
    <div className={css.container}>
      ...
      {def.cat === "input" && (
        <Input valueType={def.defaultValue.type} propId={propId} />
      )}
      {def.cat === "output" && (
        <Output valueType={def.defaultValue.type} propId={propId} />
      )}
      {prop.link && <UnlinkButton propId={propId} />}
      {prop.value.type === "number" && (
        <NumberValue disabled={def.cat === "output"} propId={propId} />
      )}
      {prop.value.type === "string" && (
        <StringValue disabled={def.cat === "output"} propId={propId} />
      )}
    </div>
  );
});
...
function UnlinkButton({ propId }: { propId: NodeId }) {
  ...
  return <div onClick={handleClick} className={css.unlinkButton} />;
}
