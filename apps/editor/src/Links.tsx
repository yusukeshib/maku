...
const Link = memo(function Link({ propId: toId }: { propId: NodeId }) {
  const { x1, y1, x2, y2 } = useAppStore((s) => {
    ...
    const toDelta =
      s.editing.dragging?.blockId === toProp.blockId
        ? s.editing.dragging.delta
        : { x: 0, y: 0 };
    ...
    const fromDelta =
      s.editing.dragging?.blockId === fromProp.blockId
        ? s.editing.dragging.delta
        : { x: 0, y: 0 };
    ...
    return {
      ...
      y1:
        fromBlock.pos.y + fromDelta.y + (fromPropIndex + 1.5) * PROPERTY_HEIGHT,
...
function DraggingLink() {
  const { isDragging, start, end } = useDragLayer((monitor) => ({
